...
from agents.rag_agent import HospitalRAG, MedicalRAG
...
class AgentConfig:
    ...
    3. HOSPITAL_INFO_AGENT – For querying hospital details, including departments, addresses, contact information, and doctor appointment availability.
    ...
    6. CHEST_XRAY_AGENT - For analysis of chest X-ray images to detect abnormalities.
...
def create_agent_graph():
    ...
    def run_rag_agent2(state: AgentState) -> AgentState:
        """Handle medical knowledge queries using RAG."""
        # Initialize the RAG agent

        print(f"Selected agent: HOSPITAL_INFO_AGENT")

        rag_agent = HospitalRAG(config)
    ...
    workflow.add_node("HOSPITAL_INFO_AGENT", run_rag_agent2)
    ...
    workflow.add_conditional_edges(
        ...
        lambda x: x["next"],
        {
            ...
            "RAG_AGENT": "RAG_AGENT",
            "HOSPITAL_INFO_AGENT": "HOSPITAL_INFO_AGENT",
    ...
    workflow.add_conditional_edges("HOSPITAL_INFO_AGENT", confidence_based_routing)
